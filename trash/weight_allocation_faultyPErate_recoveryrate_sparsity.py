...
# ---------------------
# Fig. 14：三條 sparsity 曲線，掃描 Faulty PE rate（0.1% ~ 0.5%）
# ---------------------
def run_fig14_multi_sparsity():
    array_size = 256
    sparsity_list = [30, 40, 50]
    # faulty_pe_rates = [round(x, 3) for x in np.arange(0.001, 0.006, 0.001)]  # 0.1% ~ 0.5%
    faulty_pe_rates = [round(x, 3) for x in np.arange(0.001, 0.05, 0.001)]  # 0.1% ~ 0.5%
    num_trials = 10  # 建議正式圖用 1000

    results = {}
    for sparsity in sparsity_list:
        recovery = []
        for rate in faulty_pe_rates:
            r = strict_algorithm2_repair(array_size, sparsity, rate, num_trials)
            recovery.append(r)
        results[sparsity] = recovery

    return faulty_pe_rates, results

# 執行實驗並畫圖
x_vals, recovery_data = run_fig14_multi_sparsity()

plt.figure(figsize=(8, 6))
markers = {30: 's', 40: 'o', 50: '^'}
colors = {30: 'lightgray', 40: 'gray', 50: 'black'}

for sparsity in [30, 40, 50]:
    plt.plot(
        [f'{int(x*1000)/10}%' for x in x_vals],
        recovery_data[sparsity],
...
plt.title('Recovery Rate vs. Faulty PE Rate (Varying Sparsity)', fontsize=16)
plt.ylim(0.4, 1.1)
plt.grid(True)
